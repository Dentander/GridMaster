...
class Interpreter:
    ...
    def find_all_procedures(self):
        """
        Finds all procedures in script
        """

        prev_line = -1
        while self.line < len(self.script) and not self.got_error:
            commands = self.script[self.line].split()
            if len(commands) > 0 and commands[0] == 'PROCEDURE':
                self.execute_commands(commands)
            prev_line, self.line = prev_line + 1, prev_line + 2
        self.line = 0

    def read_code_from_TextEdit(self, text):
        """
        Reads script from file
        ...
        self.script = text.split("\n")
        self.find_all_procedures()
    ...
    def execute_current_line(self):
        ...
        if len(commands) > 0 and commands[0] != 'PROCEDURE':
            self.execute_commands(commands)
        elif len(commands) > 0 and commands[0] == 'PROCEDURE':
            self.goto(self.find_block_end(self.line, 'PROCEDURE', 'ENDPROC'))
    ...
    def assert_if(self, condition: bool, error_text: str, line=None):
        ...
        self.got_error = True
        self.error = self.logger.error(error_text, line)
        return True
    ...
    def finished(self):
        return self.line >= len(self.script)
